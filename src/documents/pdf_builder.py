...
def build_executive_summary(
    ...
    pdf.cell(0, 8, "סיכום עלויות", ln=True, fill=True)
    pdf.ln(2)
    ...
    pdf.set_fill_color(245, 243, 238)
    pdf.set_font(font, 'B', 12)
    pdf.cell(0, 8, f"מבנים ({len(buildings)})", ln=True, fill=True)
    pdf.ln(2)

    type_labels = {
        'residential': 'בית מגורים', 'service': 'שירות', 'agricultural': 'חקלאי',
    ...
    if buildings:
        ...
        pdf.cell(55, 6, "שם")
        pdf.cell(35, 6, "סוג")
        pdf.cell(25, 6, 'שטח מ"ר')
        pdf.cell(30, 6, "סטטוס")
        pdf.cell(25, 6, "סטייה", ln=True)
    ...
    if tabas:
        pdf.set_fill_color(245, 243, 238)
        pdf.set_font(font, 'B', 12)
        pdf.cell(0, 8, 'תב"עות', ln=True, fill=True)
        pdf.ln(2)
    ...
    if h375 > 0 and h33 > 0:
        pdf.set_fill_color(245, 243, 238)
        pdf.set_font(font, 'B', 12)
        pdf.cell(0, 8, "השוואת מסלולי היוון", ln=True, fill=True)
        pdf.ln(2)

        pdf.set_font(font, '', 10)
        pdf.cell(0, 6, f'מסלול 3.75%: {_fmt(h375)} ש"ח', ln=True)
        pdf.cell(0, 6, f'מסלול 33%:   {_fmt(h33)} ש"ח', ln=True)
        diff = h33 - h375
        pdf.set_font(font, 'B', 10)
        pdf.cell(0, 6, f'הפרש:  {_fmt(diff)} ש"ח', ln=True)
        pdf.set_font(font, '', 9)
        if h375 < h33:
            pdf.cell(0, 5, "מסלול 3.75% זול יותר בכניסה, אך פיצול ידרוש תשלום נוסף.", ln=True)
    pdf.ln(4)
    ...
    pdf.cell(0, 4, f"הופק באמצעות מערכת בדיקת התכנות נחלות | {report_date}", ln=True, align='C')
    pdf.cell(0, 4, "הערכה ראשונית בלבד, אינה מהווה תחליף לייעוץ מקצועי", ln=True, align='C')
